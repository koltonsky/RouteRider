...
const recommendation = async (addr1, addr2) => {
  ...
  return new Promise((resolve, reject) => {  
    ...
    client.geocode(geocodeRequest1).then((response) => {
      if (response.data.status === Status.OK) {
        coords1 = response.data.results[0].geometry.location;
      } else {
        //console.error('Error for addr1: ', response.data.error_message);
      }

      client.geocode(geocodeRequest2).then((response) => {
        if (response.data.status === Status.OK) {
          coords2 = response.data.results[0].geometry.location;
        } else {
          //console.error('Error for addr2: ', response.data.error_message);
        }
        ...
        client.placesNearby(placesRequest).then((response) => {
          if (response.data.status === Status.OK) {
